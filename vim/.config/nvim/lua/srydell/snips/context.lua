...
-- path-specific contexts append extra scopes such as "cpp_test", "cpp_dsf", or
-- "python_test".
...
local function add_cpp_scopes(filetypes, path)
...
local function is_python_test_file(path)
  return path:find('/test/', 1, true) ~= nil
    or path:find('/tests/', 1, true) ~= nil
    or path:find('[_%-]test%.py$') ~= nil
    or path:find('[_%-]tests%.py$') ~= nil
end

local function add_python_scopes(filetypes, path)
  if is_python_test_file(path) then
    table.insert(filetypes, 'python_test')
  end

  return filetypes
end

local add_scopes_by_filetype = {
  cpp = add_cpp_scopes,
  python = add_python_scopes,
}
...
function M.filetypes_for_buffer(bufnr)
  ...
  -- Scopes are additive. A DSF test file intentionally gets both cpp_test and
  -- cpp_dsf snippets in addition to the base cpp snippets.
  for _, filetype in ipairs(vim.deepcopy(filetypes)) do
    local add_scopes = add_scopes_by_filetype[filetype]
    if add_scopes then
      add_scopes(filetypes, path)
    end
  end
